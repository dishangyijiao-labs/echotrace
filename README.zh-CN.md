...
以下数据基于 Apple M4 / 32 GB 实机测量（`apps/core/benchmark.py`），不同硬件会有差异。

| | 最低配置 | 推荐配置 | 高质量转录 |
|---|---|---|---|
| **Whisper 模型** | tiny | small（默认） | medium / large-v2 |
| **内存 (RAM)** | 4 GB | 8 GB | 16 GB |
| **磁盘** | 1 GB | 2 GB | 5 GB |
| **CPU** | 双核 | 四核 | 八核+ |

**各模型实测峰值内存（转录 30s 音频）：**

| 模型 | 磁盘大小 | 转录峰值内存 | CPU 峰值 | 30s 音频耗时 |
|------|---------|------------|---------|------------|
...
> tiny 模型速度最快但转录质量较差，一般不建议实际使用。small 是速度与质量的最佳平衡点。
>
> 如果开启了语义搜索，Embedding 模型（bge-small-zh-v1.5, 102 MB）会额外占用约 150 MB 内存。
>
> 选择模型时，应用会自动检测可用内存并在不足时发出提示。
